...
interface CurriculumVideo {
  ...
  youtubeId?: string;
  xpReward?: number;
  type?: string; // Some videos are marked as 'assignment'
}
...
// Video record for database insertion
 ...
 */
function extractVideosFromCurriculum(curriculum: Curriculum): VideoRecord[] {
 ...
 */
async function insertVideos(videos: VideoRecord[]): Promise<{ success: number; failed: number; errors: string[] }> {
  let success = 0;
  let failed = 0;
  const errors: string[] = [];
...
export { extractVideosFromCurriculum, insertVideos };
