  ...
  async getSimilarSuggestions(
    ...
    try {
      ...
      const { data: suggestions, error } = await supabase
        .rpc('search_similar_todo_suggestions', {
      ...
      const todoSuggestions: TodoSuggestion[] = suggestions.map((s: any) => ({
        title: s.suggestion_title,
        description: s.suggestion_description,
        priority: s.priority as 'low' | 'medium' | 'high' | 'urgent',
        category: s.category as 'practice' | 'learn' | 'build' | 'review' | 'other',
        estimatedMinutes: s.estimated_minutes,
        videoReferences: s.video_references || [],
  ...
  async getGenerationStats(userId?: string): Promise<ServiceResponse<{
    ...
    try {
      const query = supabase
        .from('chat_todo_suggestions')
        .select('accepted, confidence_score, category')
      ...
      const stats = {
        ...
        acceptanceRate: suggestions.filter(s => s.accepted).length / (suggestions.length || 1) * 100,
        averageConfidence: suggestions.reduce((sum, s) => sum + s.confidence_score, 0) / (suggestions.length || 1),
        topCategories: suggestions.reduce((acc, s) => {
